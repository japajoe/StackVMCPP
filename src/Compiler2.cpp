...
namespace StackVM
{
    ...
    bool Compiler2::Compile(const std::string& source, Assembly& assembly)
    {
    ...
    void Compiler2::RemoveWhiteSpace(std::vector<LineInfo>& lines)
    {
        for(size_t i = 0; i < lines.size(); i++)
        {
    ...
    void Compiler2::RemoveEmptyLines(std::vector<LineInfo>& lines)
    {
        auto it = lines.begin();
    ...
    void Compiler2::RemoveCommentLines(std::vector<LineInfo>& lines)
    {
        auto it = lines.begin();
    ...
    void Compiler2::RemoveTrailingComments(std::vector<LineInfo>& lines)
    {
        for(size_t i = 0; i < lines.size(); i++)
        {
    ...
    void Compiler2::Dump(const std::vector<LineInfo>& lines)
    {
        for(size_t i = 0; i < lines.size(); i++)
        {
    ...
    bool Compiler2::ProcessData(const std::vector<LineInfo>& lines, size_t offset, Assembly* assembly, std::map<std::string,uint32_t>& dataMap)
    {
        size_t dataIndex = 0;
    ...
    bool Compiler2::ProcessLabels(std::vector<LineInfo>& lines, size_t offset, Assembly* assembly, std::map<std::string,uint32_t>& labelMap)
    {
        for(size_t i = offset; i < lines.size(); i++)
        {
            if(StringUtility::EndsWith(lines[i].text, ":"))
    ...
    bool Compiler2::ProcessText(const std::vector<LineInfo>& lines, size_t offset, Assembly* assembly, std::map<std::string,uint32_t>& labelMap, std::map<std::string,uint32_t>& dataMap)
    {
        for(size_t i = offset; i < lines.size(); i++)
        {
            auto components = StringUtility::Split(lines[i].text, ' ');
    ...
    bool Compiler2::ProcessOneOperandInstruction(const LineInfo& line, OpCode opcode, const std::string operandText, Assembly* assembly, std::map<std::string,uint32_t>& labelMap, std::map<std::string,uint32_t>& dataMap)
    {
    ...
    bool Compiler2::ProcessTwoOperandInstruction(const LineInfo& line, OpCode opcode, const std::string operandTextLHS, const std::string operandTextRHS, Assembly* assembly, std::map<std::string,uint32_t>& labelMap, std::map<std::string,uint32_t>& dataMap)
    {
        byte valueLeft[8];
        byte valueRight[8];
    ...
    bool Compiler2::ParseValueFromDirective(const std::string& text, size_t lineNumber, DefineDirective d, unsigned char* buffer, Type& type)
    {
        switch(d)
        {
    ...
    Type Compiler2::GetNumberTypeFromText(const std::string& text)
    {
        bool isSigned = false;
        std::string t = text;
